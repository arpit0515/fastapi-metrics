...
- **LLM Cost Tracking**: {"✓ Enabled" if config["track_llm"] else "✗ Disabled"}
- **Alerting**: {"✓ Enabled" if config["enable_alerts"] else "✗ Disabled"}
...
5. ▢ Set up monitoring dashboard (Grafana, Retool, etc.)
...
- Docs: https://github.com/arpit0515/fastapi-metrics#readme

---
Generated by FastAPI Metrics v{__version__}
"""
...
def main():
    ...
    if config["enable_alerts"]:
        extras.append("alerts")
    install_cmd = (
        f"pip install fastapi-metrics[{','.join(extras)}]"
        if extras
        else "pip install fastapi-metrics"
    )

    code_file_line = f"✓ {code_file} - Your metrics setup code" if code_file else ""
    doc_file_line = f"✓ {doc_file} - Customized documentation" if doc_file else ""
